...
#endif
 
uniform sampler2D tInput; // The image to reproduce as a halftone
uniform vec2 resolution; // Texture dimensions, in pixels (width, height)
 
...
vec2 vOne = 1. / resolution; // 1.0/resolution, precomputed vertex shader for speed
 
...
    vec3 texcolor = texture2D_bilinear(tInput, vUv, resolution, vOne).rgb;
 
    float n = 0.1*snoise(vUv*200.0);  // Fractal noise
    n += 0.05*snoise(vUv*400.0);      // with three
    n += 0.025*snoise(vUv*800.0);     // octaves
...
#ifdef GL_OES_standard_derivatives
    float afwidth = 2.0 * frequency * max(length(dFdx(vUv)), length(dFdy(vUv)));
#else
    float afwidth = frequency * (1.0/200.0) / uScale / cos(uYrot);
#endif
