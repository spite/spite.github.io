...
void main() {
	...
	vec4 viewPos = vec4( modelViewMatrix * vec4( position + normal, 1.0 ) ); // this will transform the vertex into eyespace
    depth = 1. - ( mNear + viewPos.z ) / ( mNear - mFar );

	vPosition = vec4( modelViewMatrix * vec4( position + normal, 1.0 ) ).xyz;
